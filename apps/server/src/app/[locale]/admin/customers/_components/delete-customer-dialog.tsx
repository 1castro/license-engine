...
export function DeleteCustomerDialog({
  ...
  async function handleDelete() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/v1/customers/${customerId}`, {
        method: 'DELETE',
      });
      if (res.ok) {
        onOpenChange(false);
      ...
      if (err?.code === 'customer_has_licenses') {
        const count =
          err.details &&
          ...
          'licenseCount' in err.details
            ...
            : 0;
        setError(t('deleteHasLicenses', { count }));
      } else {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            {t('deleteConfirm', { name: customerName })}
          </DialogDescription>
        </DialogHeader>
        {error && (
