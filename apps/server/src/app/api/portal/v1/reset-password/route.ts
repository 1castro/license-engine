...
export async function POST(req: Request) {
  try {
    ...
    getLogger().error({ event: 'portal.reset_password.internal_error', err }, 'Reset-password failed');
    return jsonError(500, 'internal_error', 'Internal server error');
  }
}

async function handleResetPassword(req: Request): Promise<NextResponse> {
